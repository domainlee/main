...
class Controller implements ListenerAggregateInterface {
    ...
    {
    	$moduleName = \Base\Model\Resource::getModuleName($namespace);
    	$controllerName = \Base\Model\Resource::getControllerName($controller);
	...
	public function onDispatch(MvcEvent $event)
	{
		...
		$resource = $this->getResourceName($namespace, $controller);
		if(!$service->getAcl()->hasResource($resource)) {
			$event->getResponse()->setStatusCode(404);
		} else if ($service->isAllowed($resource, $action)) {
			return true;
		} else {
